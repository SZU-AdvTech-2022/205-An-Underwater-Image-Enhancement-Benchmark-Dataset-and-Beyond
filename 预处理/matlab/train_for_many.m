...
for i = 1:file_nums
    ...
    imwrite(image1,fullfile('test',file_dir(i).name ));
    imwrite(hazy_wb,fullfile('wbp',file_dir(i).name ));
    imwrite(hazy_cont,fullfile('clahe',file_dir(i).name ));
    imwrite(hazy_gamma,fullfile('gamma',file_dir(i).name ));
